...
class RemoteAuthRepository(context: Context) : AuthRepository {
    ...
    // SharedPreferences pour stocker la session utilisateur
    ...
    // Authentifie l'utilisateur en vérifiant d'abord en local puis via l'API si nécessaire
    override suspend fun login(email: String, password: String): Result<User> = withContext(Dispatchers.IO) {
        try {
            ...
            if (localUser != null) {
                try {
                    val apiResult = apiRepository.login(email, password)
    ...
    override suspend fun register(
        ...
        try {
            ...
            try {
                val apiResult = apiRepository.register(
                    email = email,
                    password = password,
                    firstname = firstname,
                    lastname = lastname,
